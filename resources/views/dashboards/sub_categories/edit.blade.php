<x-app-layout>
    ...
    <div class="p-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
        <form action="{{ route('sub-category.edit', [$category, $sub_category]) }}" method="POST"
            ...
            <x-Form.InputWrapper name="content">
                ...
                <x-form.input type="text" id="content" name="content" placeholder="content"
                    value="{{ $sub_category->content }}" />
            </x-Form.InputWrapper>
            <x-Form.InputWrapper name="color">
                <x-form.label for="color">{{ __('Color') }}</x-form.label>
                <x-form.input-color id="color" name="color" placeholder="color" value="{{ $sub_category->color }}" />
            </x-Form.InputWrapper>
            <button type="submit"
